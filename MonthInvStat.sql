CREATE OR REPLACE 
PROCEDURE "P_GEN_MONTH_RECORD" AS V_LASTSTAT DATE;
BEGIN
	--获取上次统计时间
	V_LASTSTAT := F_GET_RECORD_LOG('STAT_MONTH_RECORD');
	
	--记录统计日志
	WHMES."P_WRITE_STAT_lOG"('STAT_MONTH_RECORD');

	--删除临时表
	DELETE FROM TEMP_MONTH_DIMENSION;

	--存储相关新增维度数据

	INSERT INTO TEMP_MONTH_DIMENSION(BIZORGCODE,DOMAINCOL,ORGCODE,ORGSIGN,ASTUOMCODE,BIZCATEGORY,BEGINDATE,BIZTYPE,MATERIELCODE,PARTNERCODE,UOMCODE)
			...
			(
				...
				SELECT
					...
					END AS BIZCATEGORY,
					TRUNC(M.DOCDATE,'MONTH') AS BEGINDATE,
					M .DOCCAT AS BIZTYPE,
	...
	(
			SELECT 
				...
				UOMCODE 
			FROM 
				TEMP_MONTH_DIMENSION
	);
	
	--插入新的记录
	
	INSERT INTO INV_BIZSKUMONTHSTAT (
		"ID",
		"BIZORGCODE",
		"CREATEDDATE",
		"CREATORCODE",
	...
	(	
		SELECT
			...
			END AS BIZCATEGORY,
			TRUNC(M.DOCDATE,'MONTH') AS BEGINDATE,
			LAST_DAY(TRUNC(M.DOCDATE,'MONTH')) AS ENDDATE,
			SUM(CASE 
			...
			'YYYYMM' AS PERIODSIGN,
			to_number(to_char(M.DOCDATE,'MM')) AS MONTHNUM
		FROM
			Inv_TransactionLog M
		LEFT JOIN BAS_PartnerBelongComp P1 ON M.toCustomerId = P1.partnerId
		LEFT JOIN Bas_PartnerInfo P2 ON P1.partnerId = P2.ID
		...
		WHERE
			...
			CASE WHEN M. DOCTYPE = 'MOVE_PROC' AND TRANSACTIONTYPE = 'IN' THEN 'TRANS_IN' 
					 ...
					 WHEN M. DOCTYPE = 'SO_PROC' THEN 'SO'
					 WHEN M. DOCTYPE = 'ADJ_PROC' THEN 'ADJ'	
			END, TRUNC(M.DOCDATE,'MONTH'), M .DOCCAT, V .MATERIELCODE, NVL(P2.PARTNERCODE,'*'), B1.packUomCode)
			IN (
				 SELECT BIZORGCODE,
								DOMAINCOL,
								ORGCODE,
		...
		GROUP BY
			...
			B2. UOMNAME,
			TRUNC(M.DOCDATE,'MONTH'),
			LAST_DAY(TRUNC(M.DOCDATE,'MONTH')),
			to_number(to_char(M.DOCDATE,'MM'))
	);
